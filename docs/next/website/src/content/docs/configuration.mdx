...
`last_workspace` switches back to the previously focused workspace. Moving between panes or tabs inside the active workspace does not change that target. It is unset by default. Binding it to `prefix+shift+l` replaces the default `swap_pane_right` binding; choose another key if you use pane swapping.
